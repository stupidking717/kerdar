...
/**
 * Individual node item in the sidebar - n8n style
 */
function NodeItem({ nodeType, onDragStart, onClick, isLast }: NodeItemProps) {
  const handleDragStart = useCallback(
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={cn(
        'mx-2 px-3 py-2.5 cursor-grab active:cursor-grabbing',
    ...
    >
      ...
      <NodeIcon
        icon={nodeType.icon}
        color={nodeType.iconColor || '#64748B'}
        className="w-4 h-4 flex-shrink-0"
      />
      <span className="text-sm text-gray-700 dark:text-gray-200 truncate flex-1">
        {nodeType.displayName}
      </span>
      <GripVertical className="w-3 h-3 text-gray-300 dark:text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
    </div>
  );
}
